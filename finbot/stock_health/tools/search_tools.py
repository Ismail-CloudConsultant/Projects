...
def _sync_search_company_news(ticker: str, max_results: int = 5) -> dict:
...
async def search_company_news(ticker: str, max_results: int = 5) -> dict:
    """Search for recent news headlines for a ticker to assess market sentiment."""
    return await asyncio.to_thread(_sync_search_company_news, ticker, max_results)
